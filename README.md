...
`private: true` prevents accidental npm publication. GitHub source is distributed under MIT; npm publication is a separate release step. Before publishing to npm, remove `private`, rerun checks and browser regressions, and review the output of `npm pack --dry-run`. Do not include `node_modules`, unpacked Codex resources or local audit data in the public repository.
